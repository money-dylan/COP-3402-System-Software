            ...
            default:
                ...
                if(isalpha(plaintext[i])){
                    ...
                    if (strcmp(word, "begin") == 0) {
                        ...
                        strcpy(program[programSize].ID, "odd");
                        program[programSize].Token = oddsym;
                        i--;
                        programSize++;
                    }
...
/*LAYER 1: PROGRAM*/
    void Program(Lexeme *program, int programSize, Code *CodeGen, Symbol *SymbolTable){
        /*Initialize Values*/
        int Pindex = 0, PSymbolTableSize = 0, PSymbolTableIndex = 0, PGenIndex = 0;
        ...
        /*JMP 0 3*/
        CodeGenerator(CodeGen, GenIndex, 07, 0, 3);

        /*Run Token through Block until the last token*/
        while(*index < programSize){ 
            Block(CodeGen, GenIndex, index, program, SymbolTable, SymbolTableSize, SymbolTableIndex); 
        }

        /*Syntax Check: Program Ends in Period*/
        if(program[*index - 1].Token != periodsym){
        ...
        for (int i = 0; i < *SymbolTableSize; i++) {
            printf("%d %7s %7d %7d %7d %7d\n", SymbolTable[i].kind, SymbolTable[i].name, SymbolTable[i].val, SymbolTable[i].level, SymbolTable[i].addr, SymbolTable[i].mark);
        }
        return;
    }

/*RECURSION LAYER 2: DETERMINES IF TOKENS CONSTANT/VARIABLE DECLARATIONS OR STATEMENTS */
    void Block(Code *CodeGen, int *GenIndex, int *index ,struct Lexeme program[], struct Symbol SymbolTable[], int *SymbolTableSize, int *SymbolTableIndex){
        /*If current token is a constant declaration*/
        if(program[*index].Token == constsym) { ConstDec(CodeGen, GenIndex, index, program, SymbolTable, SymbolTableSize, SymbolTableIndex); } 
        
        /*If current token is a variable declaration*/
        else if(program[*index].Token == varsym) { 
            int NumVars = VarDec(CodeGen, GenIndex, index, program, SymbolTable, SymbolTableSize, SymbolTableIndex); 
            CodeGenerator(CodeGen, GenIndex, 06, 0, 3 + NumVars);
        } 
        
        /*If current token is not a declaration, it must be a statement of some type*/
        else { 
            Statement(CodeGen, GenIndex, index, program, SymbolTable, SymbolTableSize, SymbolTableIndex);
            (*index)++;
        } 
        return;
    }
...
/*RECURSION LAYER 3: HANDLES CASES OF CONSTANT & VARIABLE DECLARATIONS AND STATEMENTS*/
    ...
    void ConstDec(Code *CodeGen, int *GenIndex, int *index ,struct Lexeme program[], struct Symbol SymbolTable[], int *SymbolTableSize, int *SymbolTableIndex){
        do {
            (*index)++;
            ...
            if(SymbolTableSearch(program[*index].ID, SymbolTable, *SymbolTableSize, SymbolTableIndex) != -1){
            ...
            SymbolTableAdd(SymbolTable, 1, name, atoi(program[*index].ID), 0, SymbolTableSize,  0, SymbolTableIndex);
            (*index)++;
    ...
    int VarDec(Code *CodeGen, int *GenIndex, int *index ,struct Lexeme program[], struct Symbol SymbolTable[], int *SymbolTableSize, int *SymbolTableIndex){ //var declaration
        ...
        do {
            ...
            SymbolTableAdd(SymbolTable, 2, program[*index].ID, 0, 0, SymbolTableSize, 0, SymbolTableIndex);
            (*index)++;
    ...
    /*Statements*/
    void Statement(Code *CodeGen, int *GenIndex, int *index ,struct Lexeme program[], struct Symbol SymbolTable[], int *SymbolTableSize, int *SymbolTableIndex){
        /*Identifier*/
        if (program[*index].Token == identsym){
            /*Syntax Check: Has the Identifier already be declared*/
            int idntaddr = SymbolTableSearch(program[*index].ID, SymbolTable, *SymbolTableSize, SymbolTableIndex);           
            if(idntaddr == -1){
            ...
            (*index)++;
            Expression(CodeGen, GenIndex, index, program, SymbolTable, SymbolTableSize, SymbolTableIndex);
            CodeGenerator(CodeGen, GenIndex, 04, 0, SymbolTable[idntaddr].addr); //STO
            return;
        }
        ...
        if(program[*index].Token == ifsym){
            ...
            Condition(CodeGen, GenIndex, index, program, SymbolTable, SymbolTableSize, SymbolTableIndex);
        ...
        if(program[*index].Token == whilesym){
            ...
            CodeGenerator(CodeGen, GenIndex, 8, 0, 0); //JPC
            Statement(CodeGen, GenIndex, index, program, SymbolTable, SymbolTableSize, SymbolTableIndex);
            CodeGenerator(CodeGen, GenIndex, 07, 0, loopIdx); //JMP
...
/*RECURSION LAYER 4: CONDITIONS AND EXPRESSIONS*/
    void Condition(Code *CodeGen, int *GenIndex, int *index ,struct Lexeme program[], struct Symbol SymbolTable[], int *SymbolTableSize, int *SymbolTableIndex){
        /*ODD*/
    ...
    void Expression(Code *CodeGen, int *GenIndex, int *index ,struct Lexeme program[], struct Symbol SymbolTable[], int *SymbolTableSize, int *SymbolTableIndex){
        Term(CodeGen, GenIndex, index, program, SymbolTable, SymbolTableSize, SymbolTableIndex);
...
/*RECURSION LAYER 5: TERMS*/
    void Term(Code *CodeGen, int *GenIndex, int *index ,struct Lexeme program[], struct Symbol SymbolTable[], int *SymbolTableSize, int *SymbolTableIndex){
        Factor(CodeGen, GenIndex, index, program, SymbolTable, SymbolTableSize, SymbolTableIndex);
        /*Loop for * and / expressions*/
...
/*RECURSION LAYER 6: FACTORS*/
    void Factor(Code *CodeGen, int *GenIndex, int *index ,struct Lexeme program[], struct Symbol SymbolTable[], int *SymbolTableSize, int *SymbolTableIndex){
        if(program[*index].Token == identsym){
            /*Syntax Check: Has the Identifier already be declared*/
            int idntaddr;
            idntaddr = SymbolTableSearch(program[*index].ID, SymbolTable, *SymbolTableSize, SymbolTableIndex);           
            if(idntaddr == -1){
                printf("Error: undeclared identifier\n");
                exit(0);
            }
            ...
            if(SymbolTable[idntaddr].kind == 1){ //Const
                ...
                CodeGenerator(CodeGen, GenIndex, 03, 0, SymbolTable[idntaddr].addr); // LOD
            } 
            (*index)++;
        }
...
int SymbolTableSearch(char identifier[], struct Symbol SymbolTable[], int SymbolTableSize, int *SymbolTableIndex){
...
void SymbolTableAdd(struct Symbol SymbolTable[], int kind, char identifier[], int value, int level, int *SymbolTableSize,  int mark, int *SymbolTableIndex){
    ...
    if(kind == 1){ //constants
        ...
        SymbolTable[*SymbolTableSize].level = mark;
        (*SymbolTableSize)++;
    }
        ...
        SymbolTable[*SymbolTableSize].val = value;
        SymbolTable[*SymbolTableSize].level = level;
        SymbolTable[*SymbolTableSize].addr = 3 + *SymbolTableIndex;
        SymbolTable[*SymbolTableSize].level = mark;
        (*SymbolTableSize)++;
        (*SymbolTableIndex)++;
    }
